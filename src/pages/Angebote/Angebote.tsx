  ...
  useEffect(() => {
    const fetchOffers = async () => {
      try {
        ...
        const transformedOffers = response.data.map(
          (offer: { date: string | number | Date }) => {
            return { ...offer, date: new Date(offer.date) };
          }
        );
        setAngebote(transformedOffers);
      } catch (e) {
        ...
        {angebote.map((angebot, index) => (
          ...
          >
            ...
            <Grid item xs={12} md={6}>
              ...
              >
                <Typography>
                  {angeboteTest[index]?.car ?? "Car not available"}
                </Typography>
                <Typography>
                  {angeboteTest[index]?.name ?? "Name not available"}
                </Typography>
              </Box>
            </Grid>
          </Grid>
